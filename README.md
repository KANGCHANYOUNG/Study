...
브라우저 document.write("Tag")                  크롬 -> 인터프리터 ->Tag를안씀
 자바문법으론    out.print("")
    개행 처리 시 ln은 의미가 없다 <br>쓴다   처리할때 out.print("<br>") 
<%=%>쓴다(내장객체이기에) ;을 쓰지 않는다


<html>은                              <jsp> java +        타임라인순서
정적만 올수있다                        컴파일 일어나서(타입) -->클라이언트에 fetch 주체는 크롬이다.
fetch하고 그대로 처리                     
 쿠키 -> local                                      "유지 -->세션, 쿠키        캐쉬메모리를 쓴다. 문제는 비싸기에 용량에 제약이 있다.
